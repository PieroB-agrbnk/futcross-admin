"""
FUTCROSS | Patch 014 - Alta automatica y vencimiento honesto

Requiere los parches 001 a 013 aplicados.

1. LA REACTIVACION AHORA ES AUTOMATICA
   Cuando llega la fecha prevista de alta, el paquete vuelve a estar
   activo solo y su vencimiento se recalcula contando las sesiones que
   le quedan en el calendario de su grupo.

   Corre en dos lugares, a proposito:
     - Dentro de la base de datos, todas las noches a las 00:05 de Lima
       (pg_cron). No depende de que nadie abra nada.
     - Al abrir el panel, para que el efecto se vea al instante y no
       haya que esperar al dia siguiente.

   Si igual quieres darle de alta antes o despues de la fecha prevista,
   el boton Reactivar sigue estando y manda sobre lo automatico.

2. UN PAQUETE CONGELADO YA NO MUESTRA VENCIMIENTO
   Mientras esta pausado, la fecha que quedaba guardada era la de antes
   del congelamiento y no significa nada: se va a recalcular al dar de
   alta. Mostrarla hacia creer que el alumno vencia durante su pausa.
   Ahora esa celda va vacia, con una nota que explica por que.

IMPORTANTE
  Necesita que ANTES corras migracion-014.sql en Supabase. El archivo se
  genera solo al aplicar el parche, en esta misma carpeta.
  La migracion trae al final una consulta de verificacion que muestra
  cuantos alumnos se reactivaron en ese momento.
"""
...
PARCHE = "014-alta-automatica"
CARPETA_BACKUP = Path("respaldos") / PARCHE

# Si esta marca ya esta en app.py, el parche ya se aplico
MARCA_APLICADO = "reactivar_automaticos"
MARCA_ORIGINAL = "aun_no_empieza"
...
def aplicar():
    ...
    escribir(Path("migracion-014.sql"), MIGRACION_SQL, salto_app)

    # ---------------------------------------------------------- verificacion
    final_app, _ = leer(app)
    final_tema, _ = leer(tema)
    sql = Path("migracion-014.sql")

    texto_sql = leer(sql)[0] if sql.exists() else ""
    controles = [
        ("Alta automatica en la app", "reactivar_automaticos" in final_tema),
        ("Se ejecuta al abrir el panel", "altas_revisadas" in final_app),
        ("Calendario replicado en la base",
         "fc_fecha_de_sesion" in texto_sql),
        ("Tarea nocturna programada", "cron.schedule" in texto_sql),
        ("Congelado sin fecha de vencimiento",
         'vista["Estado"] == "CONGELADO", "Vence"' in final_app),
        ("Explica por que va vacia",
         "esa fecha se recalcula" in final_app),
        ("Migracion SQL generada", sql.exists() and sql.stat().st_size > 3000),
        ("Version 2.4", 'VERSION = "2.4"' in final_app),
    ]
    ...
    print("    1. Abre migracion-014.sql (quedo en esta carpeta)")
